...
impl ServiceWorkerManager {
    ...
    pub fn spawn_manager(sw_senders: SWManagerSenders) {
        ...
        spawn_named("ServiceWorkerManager".to_owned(), move || {
            ServiceWorkerManager::new(own_sender, from_constellation, resource_port, constellation_sender).handle_message();
        });
    }
    ...
    pub fn prepare_activation(&mut self, load_url: &Url) -> Option<Sender<ServiceWorkerScriptMsg>> {
        ...
        if let Some(ref scope_url) = scope_url {
            ...
            if let Some(scope_things) = scope_things {
                ...
                let trusted_msg = SWManagerMsg::NetworkFetch(scope_url.clone(), scope_things.pipeline_id);
                let _ = self.constellation_sender.send(trusted_msg);
